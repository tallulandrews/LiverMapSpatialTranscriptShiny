...
#### My Varimax ####
RunVarimax <- function(seur_obj, npcs=12, do.scale=TRUE, use.projected = TRUE) {
	require(Seurat)
...
for (f in files) {
	...
	# Gather loadings for matching
	loading_mat <- obj@reductions$varimax@feature.loadings

	if (length(varimax_gene_loadings_matrix) == 0) {
		varimax_gene_loadings_matrix <- loading_mat
	} else {
		all_genes <- sort(unique(c(rownames(varimax_gene_loadings_matrix), rownames(loading_mat))))
		varimax_gene_loadings_matrix <- varimax_gene_loadings_matrix[match(all_genes, rownames(varimax_gene_loadings_matrix)),]
		loading_mat <- loading_mat[match(all_genes, rownames(loading_mat)),]
		rownames(varimax_gene_loadings_matrix) <- all_genes
		rownames(loading_mat) <- all_genes
		varimax_gene_loadings_matrix <- cbind(varimax_gene_loadings_matrix, loading_mat)
	}

	for (i in 1:npcs) {
		scores <- loading_mat[,i]
		scores <- sort(scores, decreasing=TRUE)
		top_up_gene_lists <- cbind(top_up_gene_lists, head(names(scores),ntop))
		top_down_gene_lists <- cbind(top_down_gene_lists, tail(names(scores),ntop))
...
stuff <- readRDS("Spatial_varimax_clustering.rds")
varimax_gene_loadings_matrix <- stuff$varimax_gene_loadings_matrix;
diff_int <- stuff$diff_intersection;
diff_cor <- stuff$diff_correlation
cluster_lab <- stuff$clusters_int
hmap <- stuff$heatmap_obj_int
...
do_fgsea(scores, react_gmt)

out3[out3$padj < 0.05 & out3$NES > 0,]

tmp <- out3[out3$padj < 0.05 & out3$NES > 0,]; tmp <- tmp[order(tmp$NES),]; print(tmp); unlist(tmp[12,8])
...
do_fgsea <- function(scored_genes, pathways=MSigAll, fdr=0.05, nmax=20){
	...
	require(igraph)
	G <- simplify(graph_from_adjacency_matrix(sim_mat > 0.5, mode="undirected"))
